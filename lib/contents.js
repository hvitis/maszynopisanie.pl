...
export const getListPage = async (folder) => {
  ...
  const sanitizeIndexFile = indexPath.filter((file) =>
    file.includes('.md')
  );
  const filterIndexFile = sanitizeIndexFile.filter((file) =>
    file.match(/^_/)
  );
...
export const getRegularPage = async (slug) => {
  ...
  const pageData = allPublishedPages.filter(
    (data) => data.slug === slug
  );
